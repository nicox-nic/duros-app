...
export default function NewGatePassPage() {
  ...
  const [hostId, setHostId] = useState(residents[0]?.id ?? '');
  ...
  const selectedHost = residents.find((r) => r.id === hostId);
  const requiresPlate = ['delivery', 'contractor', 'move_in', 'move_out', 'furniture', 'vehicle'].includes(type);
  const canSubmit = visitorName.trim() && selectedHost && (!requiresPlate || plateNumber.trim());

  const handleSubmit = () => {
    if (!canSubmit || !selectedHost || !property) return;
    setSubmitting(true);
    ...
    createGatePass({
      ...
      hostUnit: selectedHost.unitNumber!,
    ...
    setShowSuccess(true);
    setTimeout(() => router.push('/staff/permits/gate'), 1200);
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-none md:overflow-visible">
      <div className="md:max-w-[680px] md:mx-auto md:px-8 md:py-8">
        <PageHeader
          title="New Gate Pass"
          backHref="/staff/permits/gate"
          description="Approve entry for a visitor, delivery, or contractor."
        />
        ...
        <FieldLabel>Host Resident / Unit</FieldLabel>
        <Select
          value={hostId}
        ...
        <div className="fixed inset-0 z-[200] bg-charcoal/40 backdrop-blur-sm grid place-items-center p-6">
          <div className="bg-white rounded-3xl shadow-soft-lg p-6 md:p-8 text-center max-w-sm">
            ...
            <div className="text-[12px] md:text-[13.5px] text-mist">
              QR code generated. Security and host notified.
            </div>
          </div>
        </div>
      )}
